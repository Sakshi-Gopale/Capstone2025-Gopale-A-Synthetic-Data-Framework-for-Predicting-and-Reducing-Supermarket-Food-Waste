...
for pid, group in df.groupby("product_id"): #Looping over each product
    ...
    last_date = group['date'].max() #Last known date of the product
    future_dates = pd.date_range(pd.to_datetime("2025-12-01") + pd.Timedelta(days=1), periods=forecast_duration) #Last date can be changed 
...
#Building a future dataframe using the last known product attributes
    future_df = pd.DataFrame({
        ...
        'unit_price': group['unit_price'].iloc[-1], #Using last know unit price
...
#Calculating split index manually
split_idx = int(len(X) * (1 - test_size))

X_train, X_test = X.iloc[:split_idx], X.iloc[split_idx:]
y_train, y_test = y.iloc[:split_idx], y.iloc[split_idx:]
test_dates = dates.iloc[split_idx:]   #Aligned dates
...
plt.title(f"30-Day Forecast for Product {sample_pid}")
plt.xlabel("Date")
plt.ylabel("Forecast Quantity")
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("forecast_30days.png", dpi=300)
plt.close()
