...
def infer_types(df: pd.DataFrame, target: Optional[str], date_col: Optional[str]) -> Tuple[List[str], List[str]]:
    ...
    drop = set([c for c in [target, date_col] if c])
    if date_col and date_col in df.columns and not np.issubdtype(df[date_col].dtype, np.datetime64):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
    numeric_cols = [c for c in cols if c not in drop and pd.api.types.is_numeric_dtype(df[c])]
    categorical_cols = [c for c in cols if c not in drop and c not in numeric_cols]
...
def add_lag_rolling(df: pd.DataFrame, date_col: Optional[str], id_cols: List[str], target: str,
    ...
    if not date_col or not id_cols:
        return df
    df = df.copy()
    if not np.issubdtype(df[date_col].dtype, np.datetime64):
        df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
...
def bin_target_for_stratify(y: pd.Series, q: int = 5) -> np.ndarray:
    ...
    try:
        ...
        if len(pd.unique(bins.dropna())) < 2:
...
def get_cv_splits(X: pd.DataFrame, y: pd.Series, cv_type: str, folds: int,
    ...
    if cv_type == "timeseries":
        ...
        order = np.argsort(pd.to_datetime(X[date_col], errors="coerce").values)
...
def train_one_model(model_name: str, Xtr, ytr, Xva, yva, input_dim=None):
    if model_name == "rf":
        ...
        cb = [keras.callbacks.EarlyStopping(monitor="val_mae", patience=15, restore_best_weights=True)]
...
def cross_validate(X: pd.DataFrame, y: pd.Series, cv_type: str, folds: int, models: List[str],
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(get_cv_splits(X, y, cv_type, folds, groups, date_col), start=1):
        Xtr_raw, Xva_raw = X.iloc[tr_idx].copy(), X.iloc[va_idx].copy()
        ytr, yva = y.iloc[tr_idx].astype(float), y.iloc[va_idx].astype(float)
...
def classification_metrics(y_true: np.ndarray, y_pred: np.ndarray, scheme: str = "zero_vs_nonzero", bins: int = 3):
    if scheme == "zero_vs_nonzero":
        ...
        labels = sorted(pd.unique(y_true_cls.dropna()).tolist())
...
def save_residual_plots(y_true, y_pred, out_png_hist: Path, out_png_scatter: Path, title: str):
    resid = y_true - y_pred
    plt.figure()
    plt.hist(resid, bins=30)
    plt.title(f"Residual Histogram — {title}")
    plt.xlabel("Residual")
    plt.ylabel("Count")
    plt.tight_layout()
    plt.savefig(out_png_hist)
    plt.close()
...
def plot_confusion_matrix(cm: np.ndarray, labels: List[int], out_png: Path, title: str):
    plt.figure()
...
def plot_prf1(per_class: List[dict], macro: dict, out_macro_png: Path, out_perclass_png: Path, title: str):
    # Macro bar
    plt.figure()
    metrics = ["precision", "recall", "f1"]
    values = [macro.get("precision", 0), macro.get("recall", 0), macro.get("f1", 0)]
    plt.bar(metrics, values)
    plt.ylim(0, 1.0)
    plt.title(f"Macro Precision/Recall/F1 — {title}")
    plt.ylabel("Score")
    plt.tight_layout()
    plt.savefig(out_macro_png, dpi=200, bbox_inches="tight")
    plt.close()

    # Per-class grouped bars
    labels = [str(pc["label"]) for pc in per_class]
    p_vals = [pc["precision"] for pc in per_class]
    r_vals = [pc["recall"] for pc in per_class]
    f_vals = [pc["f1"] for pc in per_class]

    x = np.arange(len(labels))
    width = 0.25
    plt.figure()
    plt.bar(x - width, p_vals, width, label="Precision")
    plt.bar(x, r_vals, width, label="Recall")
    plt.bar(x + width, f_vals, width, label="F1")
    plt.xticks(x, labels)
    plt.ylim(0, 1.0)
    plt.title(f"Per-Class P/R/F1 — {title}")
    plt.xlabel("Class")
    plt.ylabel("Score")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_perclass_png, dpi=200, bbox_inches="tight")
    plt.close()


def plot_metrics_summary(reg: dict, cls: Optional[dict], out_png: Path, title: str):
    lines = [f"MAE: {reg.get('MAE', np.nan):.4f}",
             f"RMSE: {reg.get('RMSE', np.nan):.4f}",
             f"R²: {reg.get('R2', np.nan):.4f}"]
    if cls:
...
def plot_model_comparison(models: List[str], metrics_list: List[dict], outdir: Path):
    # Separate PNGs for MAE / RMSE / R2
    keys = [("MAE", "model_comparison_mae.png"),
            ("RMSE", "model_comparison_rmse.png"),
            ("R2", "model_comparison_r2.png")]
...
def main():
    ...
    p.add_argument("--models", nargs="+", default=["rf", "xgb", "dl"], choices=["rf", "xgb", "dl"])
    p.add_argument("--no_scale_numeric", action="store_true")
    p.add_argument("--save_template", action="store_true")
    p.add_argument("--cv_type", choices=["kfold", "groupkfold", "timeseries", "stratkfold"], default="kfold")
    ...
    p.add_argument("--cls_bins", type=int, default=3)
    args = p.parse_args()
    ...
    for mname in args.models:
        try:
            ...
            comparison_rows.append({"model": mname, **reg_metrics})
            comparison_models.append(mname)
            comparison_metrics.append(reg_metrics)

            resid_df = pd.DataFrame({"y_true": y_val, "y_pred": y_pred, "residual": y_val - y_pred})
            resid_df.to_csv(analysis_dir / f"residuals_{mname}.csv", index=False)

            save_residual_plots(y_val, y_pred, analysis_dir / f"residual_hist_{mname}.png",
                                analysis_dir / f"actual_vs_pred_{mname}.png", title=mname.upper())

            #Classification-style metrics + images
            cls_result = None
            if args.cls_scheme != "none":
                ...
                cm = np.array(cls_result["confusion_matrix"])
                labels = cls_result["labels"]
                plot_confusion_matrix(cm, labels, analysis_dir / f"confusion_matrix_{mname}.png", title=mname.upper())
                # PR/F1 images
                plot_prf1(cls_result["per_class"], cls_result["macro"],
                          analysis_dir / f"prf1_macro_{mname}.png",
                          analysis_dir / f"prf1_per_class_{mname}.png",
                          title=mname.upper())
            ...
            #Feature importances CSVs (optional)
            if mname == "rf":
                try:
                    fi = getattr(mdl, "feature_importances_", None)
                    if fi is not None:
                        pd.DataFrame({"feature_index": np.arange(len(fi)), "importance": fi}).to_csv(
                            analysis_dir / "feature_importance_rf.csv", index=False
                        )
                except Exception:
                    pass
            if mname == "xgb":
                try:
                    ...
                    score = booster.get_fscore()
                    pd.DataFrame({"feature": list(score.keys()), "importance": list(score.values())}).sort_values(
                        "importance", ascending=False
                    ).to_csv(analysis_dir / "feature_importance_xgb.csv", index=False)
                except Exception:
                    pass
    ...
    #Model comparison CSV and PNGs
    if comparison_rows:
        pd.DataFrame(comparison_rows).to_csv(outdir / "model_comparison.csv", index=False)
        plot_model_comparison(comparison_models, comparison_metrics, analysis_dir)
    ...
    print(json.dumps({"cv_results": cv_results, "holdout_metrics": final_metrics,
